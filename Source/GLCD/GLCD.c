...
void DisplayInteger(uint16_t Xpos, uint16_t Ypos, int value, uint16_t Color, uint16_t bkColor)
{
    char buffer[12]; // Buffer per contenere la stringa dell'intero
    sprintf(buffer, "%d", value); // Converte l'intero in una stringa
    GUI_Text(Xpos, Ypos, (uint8_t*)buffer, Color, bkColor); // Passa la stringa a GUI_Text
}
