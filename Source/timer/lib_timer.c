...
uint32_t init_timer ( uint8_t timer_num, uint32_t Prescaler, uint8_t MatchReg, uint8_t SRImatchReg, uint32_t TimerInterval )
{
  ...
  else if ( timer_num == 1 )
  {
	...
	NVIC_SetPriority(TIMER1_IRQn, 1);	/* less priority than RIT*/
	return (0);
  }
// TIMER 2
	  else if ( timer_num == 2 )
  {
	...
	NVIC_SetPriority(TIMER2_IRQn, 1);	/* less priority than RIT*/
	return (0);
  }
// TIMER 3
	  else if ( timer_num == 3 )
  {
	...
	NVIC_SetPriority(TIMER3_IRQn, 1);	/* less priority than RIT*/
	return (0);
  }
